function f_pert = get_srp_pert_force(Rpe,Rse,eclipse,sc_cfg,sys_cfg)
    ...
    % the net force by drag-like forces normal to faces applies no net
    % torque on the body (this holds for the rectangular prism assumption,
    % despite that this is counter-intuitive). Therefore we approximate the
    % normal unit vector as the unit vector from the spacecraft to the Sun.
    % This means that the SRP equation, as described in the slides,
    % reduces:
    ...
    F_pert = -sys_cfg.earth.SRP * (sys_cfg.km_per_AU)^2/rsp^2 * ...
             sc_cfg.As * ((1+sc_cfg.ep)*rsp_hat) / 1000; % [kN]
